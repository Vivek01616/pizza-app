...
function AdminDashboard() {
  ...
  const fetchPizzas = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setPizzas(data);
        const availablePizzasData = {};
        data.forEach(pizza => {
          availablePizzasData[pizza._id] = pizza.quantity; // Assuming there's a 'quantity' field for each pizza
        });
        setAvailablePizzas(availablePizzasData);
      } else {
  ...
  const handleOrderCheck = (selectedPizza, quantity) => {
    if (!selectedPizza || availablePizzas[selectedPizza._id] < quantity) {
      setNotification('Requested pizza is not available or quantity exceeds available stock.');
      return false;
    }
    return true;
  };
            ...
            <div>
              ...
              <div className="panel black-panel">
                <div className="panel-body">
                  ...
                  <button className="btn btn-primary btn-block mt-2" onClick={() => handleOrderCheck(pizzas.find(pizza => pizza.name === pizzaName), 1) && handleAddPizza()}>Add Pizza</button>
                ...
                {orders.map(order => (
                  <li key={order._id} className="list-group-item d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      Status:
                      <select value={order.orderStatus} onChange={(e) => handleUpdateOrderStatus(order._id.toString(), e.target.value)}>
